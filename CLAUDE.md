...
- `/components`: Nav (wit op de herofoto van de voorpagina, daarna antraciet op wit), Footer, Scroll (Voortgang, MobieleBalk), Hero over de volle hoogte met Diashow (drie foto's die het kader vullen, wisselen elke 3,5 seconde, stippen rechtsonder, stil bij reduced motion, donkere waas onderin voor de leesbaarheid), Home (VoorWie, Amsterdam, DuoSolo, Samen, Stappen, Inbegrepen, LeasenOfDelen, Slot), Prijzen (Prijstabel, Beschikbaarheid, KostenPerVaart, Overeenkomst), ReserveerFormulier, ProefvaarFormulier, PrintKnop.
...
- Typografie: koppen in Instrument Serif 400, regelhoogte 1,0, h1 46 px mobiel tot 84 px desktop (hero 52 tot 104 px, wit op de foto), met één cursief accent via `*woord*` in de herokop (blauw op wit, gebroken wit op de foto). h3 in Inter 600 of via `kop` in de serif. Sectielabels als kleine kapitalen (`label`, 12 px, 0.14em). Body Inter 17 tot 18 px, regelhoogte 1,5, maximaal 65 tekens (`maat`). Geen bold in lopende tekst.
...
- Componenten: sticky nav transparant, wit met dunne onderlijn bij scrollen. Eén primaire knop (blauw, pil; variant `licht` op donker), secundair als tekstlink met pijl (`licht` op donker). Radius `knop` pil, `kaart` 20 px voor foto's in de pagina, met een hairline aan de binnenkant (`Foto`). De hero loopt van rand tot rand. Schaduw maximaal `shadow-licht`. Accordion met dunne lijnen (CSS grid-rows). Formulieren: label boven het veld, grote velden, duidelijke focus, één knop.
