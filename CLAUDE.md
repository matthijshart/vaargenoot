...
Geen schoonmaakservice: sloepmaten laten hem zelf opgeruimd achter, het is
hun eigen sloep. Wij laden op en doen het onderhoud.
...
dragen de pagina. Grote strakke koppen, monospace-labels, felblauwe
...
- Componenten: sticky nav wit met de links naast het woordmerk, dunne onderlijn bij scrollen. Nooit tekst op een foto: de hero zet label, kop en uitleg op wit, de foto eronder in één breed kader. Knoppen als pil: `blauw` primair, ook op de donkere kaart, `licht` (room) ernaast, `wit` alleen als het blauw niet kan; tekstlink in blauw met pijl ("Bekijk Duo", "Zo werkt het") voor links in een blok. Nooit een zwarte knop. Radius `knop` pil, `kaart` 28 px, foto's met een hairline aan de binnenkant (`Foto`). Schaduw maximaal `shadow-licht`. Accordion met dunne lijnen (CSS grid-rows). Formulieren: label boven het veld, grote velden, duidelijke focus, één knop.
