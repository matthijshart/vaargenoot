...
- `/components`: Nav, Footer, Scroll (Voortgang, MobieleBalk), Hero met Diashow (drie foto's, wisselen elke 3,5 seconde, stippen om te kiezen, stil bij reduced motion), Home (VoorWie, Amsterdam, DuoSolo, Samen, Stappen, Inbegrepen, LeasenOfDelen, Slot), Prijzen (Prijstabel, Beschikbaarheid, KostenPerVaart, Overeenkomst), ReserveerFormulier, ProefvaarFormulier, PrintKnop.
...
- Typografie: koppen in Instrument Serif 400, regelhoogte 1,0, h1 46 px mobiel tot 84 px desktop (hero 92 px), met één cursief accent in blauw via `*woord*` in de herokop. h3 in Inter 600 of via `kop` in de serif. Sectielabels als kleine kapitalen (`label`, 12 px, 0.14em). Body Inter 17 tot 18 px, regelhoogte 1,5, maximaal 65 tekens (`maat`). Geen bold in lopende tekst.
...
- Componenten: sticky nav transparant, wit met dunne onderlijn bij scrollen. Eén primaire knop (blauw, pil), secundair als tekstlink met pijl. Radius `knop` pil, `kaart` 20 px; de hero-foto 20 tot 28 px in een breed kader. Schaduw maximaal `shadow-licht`. Accordion met dunne lijnen (CSS grid-rows). Formulieren: label boven het veld, grote velden, duidelijke focus, één knop.
